...
export function ProductActions({ product }: { product: MerchantProduct }) {
  ...
  return (
    <div
      ref={actionGroup}
      tabIndex={-1}
      className="mb-6 space-y-3 rounded-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus"
    >
      {confirmed ? (
        <div key="confirmed-actions" className="space-y-3">
          <p role="status" className="text-sm text-text-muted">
            {mutation.state.action === "publish"
              ? "Product published."
              : mutation.state.action === "unpublish"
                ? "Product unpublished."
                : "Product saved."}{" "}
            Review the latest product before another action.
          </p>
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                if (await mutation.reviewSuccess()) actionGroup.current?.focus();
              }}
            >
        ...
        <div key="available-actions" aria-label="Product actions" className="flex flex-wrap gap-2">
